import React from 'react';

const MarylandRestaurantNovi = () => {
  return (
    <div className=" py-20 relative">
      {/* Background Image with Overlay */}
      <div className="absolute inset-0 z-0">
        <div className="absolute inset-0 bg-linear-to-br from-blue-900/30 to-indigo-900/20"></div>
        <div 
          className="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1730782396362-a77e14d6f009?q=80&w=3432&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D')] bg-cover bg-center"
       
        ></div>
        <div className="absolute inset-0 bg-green-500/20 backdrop-blur-[100px]"></div>
      </div>

      {/* Content */}
      <div className="relative z-10">
        <div className="container px-4 md:px-0 mx-auto">
              <h2 className="text-3xl md:text-5xl font-serif max-w-4xl mx-auto text-center font-bold text-black mb-6 leading-tight">
                Get Your Retail Novi Eats Online Ordering Today
              </h2>

          {/* Main Content Grid */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-16 mt-16">
            {/* Left Column - Consultation Services */}
            <div>
              <div className="glass-morphism relative overflow-hidden bg-blue-600/3 p-8 min-h-full">
                <div className="relative z-10">
                  <div className="flex items-center mb-8">
                    <h2 className="text-2xl md:text-3xl font-bold text-gray-800">
                    Free Retail Consultation Includes:                </h2>
                  </div>
                  
                  <div className="space-y-6">
                    <ConsultationItem 
                      title="Retail business needs assessment and POS system recommendation"
                      description=""
                      feature="Personalized Analysis"
                    />
                    <ConsultationItem 
                      title="	Current retail payment processing evaluation and cost analysis"
                      description=""
                      feature="Tailored Solutions"
                    />
                    <ConsultationItem 
                      title="	Retail credit card processing pricing comparison and savings calculation"
                      description=""
                      feature="Cost Optimization"
                    />
                    <ConsultationItem 
                      title="Inventory management and customer loyalty program planning"
                      description=""
                      feature="Transparent Pricing"
                    />
                    <ConsultationItem 
                      title="Multi-location coordination and equipment integration planning"
                      description=""
                      feature="Easy Integration"
                    />
               
                  </div>
                </div>
              </div>
            </div>

            {/* Right Column - Benefits */}
            <div>
              <div className="glass-morphism  relative overflow-hidden bg-blue-600/3 p-8 in-h-full">
                <div className="relative z-10">
                  <div className="flex items-center mb-8">
                    <h2 className="text-2xl md:text-3xl font-bold text-gray-800">Why Choose Elite Card Processing for Retail Merchant Services:</h2>
                  </div>
                  <div className="space-y-6">
                    <BenefitItem 
                      title="	Retail payment processing company with responsive, dedicated support"
                      description=""
                      highlight=""
                    />
                    <BenefitItem 
                      title="Professional retail equipment installation and comprehensive staff training"
                      description=""
                      highlight=""
                    />
                    <BenefitItem 
                      title="Competitive retail credit card processing rates with transparent pricing"
                      description=""
                      highlight=""
                    />
                    <BenefitItem 
                      title="	Integration with cash discount programs for retail businesses"
                      description=""
                      highlight=""
                    />
                    <BenefitItem
                      title="Inventory management and customer loyalty program support included"
                      description=""
                      highlight=""
                    />
                    <BenefitItem 
                      title="Scalable retail payment processing solutions that grow with your business"
                      description=""
                      highlight=""
                    />
                  </div>
                </div>
              </div>
            </div>
          </div>

          {/* Contact CTA beneath boxes */}
          <div className="text-center mt-12">
            <a 
              href="/contact" 
              className="inline-flex items-center justify-center gap-2 px-8 py-4 bg-[#10284D] text-white font-semibold rounded-xl shadow-lg hover:bg-[#152E5A] transition-all duration-300 hover:-translate-y-0.5"
            >
              Contact Us
            </a>
          </div>
        </div>
      </div>
    </div>
  );
};

// Consultation Item Component
const ConsultationItem = ({ title, description, feature }: { title: string; description: string; feature: string }) => (
  <div className="flex items-start group cursor-pointer">
     <div className="shrink-0 mt-3 w-2 h-2 bg-gray-400 rounded-full">
     </div>
    <div className="ml-4 flex-1">
      <div className="flex items-center justify-between">
        <h4 className="font-bold text-gray-800 group-hover:text-white transition-colors duration-300">{title}</h4>
      </div>
      <p className="text-gray-600 text-sm mt-1">{description}</p>
    </div>
  </div>
);

// Benefit Item Component
const BenefitItem = ({ title, description, highlight }: { title: string; description: string; highlight: string }) => (
  <div className="flex items-start group cursor-pointer">
   <div className="shrink-0 mt-3 w-2 h-2 bg-gray-400 rounded-full">
   </div>
    <div className="ml-4 flex-1">
      <div className="flex items-center justify-between">
        <h4 className="font-bold text-gray-800 group-hover:text-white transition-colors duration-300">{title}</h4>
      
      </div>
      <p className="text-gray-600 text-sm mt-1">{description}</p>
    </div>
  </div>
);


export default  MarylandRestaurantNovi;